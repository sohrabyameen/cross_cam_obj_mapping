...
class first_vid:
    def player_detection(self, results):

        out_dir = "output_A"
        video_out_path = os.path.join(out_dir, "video_A_annotated.avi")

        height, width = results[0].orig_img.shape[:2]
        fps = 24  # or extract from original video
        fourcc = cv2.VideoWriter_fourcc(*"XVID")
        out = cv2.VideoWriter(video_out_path, fourcc, fps, (width, height))


        # Dictionary to hold crops per ID
    ...
    def embedding_extraction(self,model, transform):
        player_gallery = {}
...
class second_vid:
    def player_mapping(self, results, player_gallery, model, transform):
        ...
        # Mapping from Video B tracking ID to matched global ID
        b_to_global_id = {}
        for result in results:
            ...
            for box in boxes:
                ...
                # Threshold for matching (tune if needed)
                if best_score > 0.75:
